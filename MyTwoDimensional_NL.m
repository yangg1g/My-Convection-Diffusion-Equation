...
%             fi=Yx*f(j,k)+(Yy-Yx)*(-(u(j,k+1)-2*u(j,k)+u(j,k-1))/h/h+aiy*(u(j,k+1)-u(j,k-1))/2/h);
            fi=Yx*f(j,k)+Yx_*(f(j+1,k)-f(j-1,k))/2/h+Yy_*(f(j,k+1)-f(j,k-1))/2/h;
            fi=fi+Yx_*((u(j+1,k+1)-2*u(j+1,k)+u(j+1,k-1))/h/h-(u(j-1,k+1)-2*u(j-1,k)+u(j-1,k-1))/h/h)/2/h;
            fi=fi+Yy_*((u(j+1,k+1)-2*u(j,k+1)+u(j-1,k+1))/h/h-(u(j+1,k-1)-2*u(j,k-1)+u(j-1,k-1))/h/h)/2/h;
%            
