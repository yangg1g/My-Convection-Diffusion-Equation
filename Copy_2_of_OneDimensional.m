...
%     U0=[Y_(1),Y_(n+1)];
    ...
    [U,pp,ee]=Solve1__(n,1e-2);
    E1(i)=max(abs(ee));
    [U,pp,ee]=Solve2__(n,1e-2);
    E2(i)=max(abs(ee));
    n=n*2;
    i=i+1;
end
...
fprintf("|%d|%d||%d||\n",i*4,E1(i-1),E2(i-1));
while i<=7
    e1(i-1)=log(E1(i-1)/E1(i))/log(2);
    e2(i-1)=log(E2(i-1)/E2(i))/log(2);
    fprintf("|%d|%d|%d|%d|%d|\n",2^(i+2),E1(i),e1(i-1),E2(i),e2(i-1));
    i=i+1;
end
% plot(X_,E);
% hold on;
...
function [u,p,e,A2]=Solve1__(n,e)
h=1/n;
x=0:h:1;
a=zeros(1,n+1);
f=zeros(1,n+1);
...
for i=2:n
    ...
    Y(i-1)=Y0_2(ri,a(i),ax(i),e)*f(i)+Y1_2(ri,a(i),ax(i),e)*(f(i+1)-f(i-1))/4/h;
end
%t=A1;
Y(1)=Y(1)-A1(1)*p(1);
A1(1)=[];
...
%r=2 - x*(2*x - 1);
y=1/128;
r=(exp(x) + (101*(x + 1)^100)/1267650600228229401496703205376)*(exp(y) + (y + 1)^101/1267650600228229401496703205376)*(x + 1) - ((exp(x) + (2525*(x + 1)^99)/316912650057057350374175801344)*(exp(y) + (y + 1)^101/1267650600228229401496703205376))/100;
end
...
%r=1 - 4*x;
y=1/128;
r=(exp(x) + (101*(x + 1)^100)/1267650600228229401496703205376)*(exp(y) + (y + 1)^101/1267650600228229401496703205376) - ((exp(x) + (249975*(x + 1)^98)/316912650057057350374175801344)*(exp(y) + (y + 1)^101/1267650600228229401496703205376))/100 + (exp(x) + (2525*(x + 1)^99)/316912650057057350374175801344)*(exp(y) + (y + 1)^101/1267650600228229401496703205376)*(x + 1);
end
